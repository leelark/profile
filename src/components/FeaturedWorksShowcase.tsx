...
export default function FeaturedWorksShowcase({
  ...
  return (
    <>
      <section className="key-stage" aria-label="Featured work overview">
        <div className="key-stage-visual" aria-hidden="true">
          ...
          {spotlightItems.slice(0, 4).map((item, index) => {
            ...
            return (
              <span
                className={`key-stage-node key-stage-node-${index + 1} key-tone-${item.tone}`}
                key={item.sourceSlug}
      ...
      <section className="key-plugin-lab" aria-label="Appian plugin portfolio">
